...
export function HeroSection({ title, subtitle, image, actions }: { title: string; subtitle: string; image: string; actions?: ReactNode }) {
  return (
    <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-olive-900/60 px-6 py-16 md:px-12 md:py-20">
      <div className="pointer-events-none absolute inset-0 opacity-60">
        <div className="absolute -left-20 top-0 h-80 w-80 rounded-full bg-gold-500/20 blur-[120px] animate-aurora" />
        <div className="absolute -right-20 bottom-0 h-80 w-80 rounded-full bg-forest-500/20 blur-[120px] animate-aurora" />
      </div>
      ...
      <div className="relative grid items-center gap-10 md:grid-cols-[1.1fr_1fr]">
        <div>
          ...
          <p className="mb-8 max-w-xl text-lg text-olive-100">{subtitle}</p>
          {actions}
        </div>
